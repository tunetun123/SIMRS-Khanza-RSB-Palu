...
public final class DlgRl38 extends javax.swing.JDialog {
    ...
    private int i = 0, a = 0, ttl = 0, jumlahl = 0, jumlahp = 0, ttljumlahl = 0, ttljumlahp = 0, nilaitotall = 0,
            nilaitotalp = 0, ttlnilaitotall = 0, ttnilaip = 0, jumlahpasienl = 0, jumlahpasienp = 0, ratal = 0,
            ratap = 0, ttlratal = 0, ttlratap = 0;
    ...
    public DlgRl38(java.awt.Frame parent, boolean modal) {
        ...
        try {
            /*
             * pstindakan=koneksi.
             * prepareStatement("select jns_perawatan_lab.nm_perawatan,count(jns_perawatan_lab.nm_perawatan),jns_perawatan_lab.kd_jenis_prw from periksa_lab "
             * +
             ...
             * );
             * pstindakan2=koneksi.
             * prepareStatement("select template_laboratorium.Pemeriksaan,count(template_laboratorium.Pemeriksaan) from detail_periksa_lab "
             * +
            ...
            pstindakan = koneksi.prepareStatement(
                    "select jns_perawatan_lab.nm_perawatan,jns_perawatan_lab.kd_jenis_prw from periksa_lab "
                            + "inner join jns_perawatan_lab on periksa_lab.kd_jenis_prw=jns_perawatan_lab.kd_jenis_prw "
                            + "where jns_perawatan_lab.nm_perawatan like ? group by jns_perawatan_lab.nm_perawatan ");
            pstindakan2 = koneksi.prepareStatement(
                    "select template_laboratorium.Pemeriksaan,template_laboratorium.kd_jenis_prw from detail_periksa_lab "
                            + "inner join template_laboratorium on detail_periksa_lab.id_template=template_laboratorium.id_template "
        ...
        try {
            ...
            while (rstindakan.next()) {
                ...
                while (rstindakan2.next()) {
                    jumlahl = Sequel.cariInteger(
                            "select count(template_laboratorium.Pemeriksaan) FROM template_laboratorium INNER JOIN detail_periksa_lab "
                                    ...
                                    + "where detail_periksa_lab.tgl_periksa between '"
                                    + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' and '"
                                    + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' "
                                    + "and template_laboratorium.kd_jenis_prw='" + rstindakan2.getString("kd_jenis_prw")
                                    + "' and pasien.jk = 'L' group by template_laboratorium.Pemeriksaan ");
                    ttljumlahl = ttljumlahl + jumlahl;

                    jumlahp = Sequel.cariInteger(
                            "select count(template_laboratorium.Pemeriksaan) FROM template_laboratorium INNER JOIN detail_periksa_lab "
                                    + "ON template_laboratorium.id_template = detail_periksa_lab.id_template INNER JOIN reg_periksa ON detail_periksa_lab.no_rawat = reg_periksa.no_rawat INNER JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis "
                                    + "where detail_periksa_lab.tgl_periksa between '"
                                    + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' and '"
                                    + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' "
                                    + "and template_laboratorium.kd_jenis_prw='" + rstindakan2.getString("kd_jenis_prw")
                                    + "' and pasien.jk = 'P' group by template_laboratorium.Pemeriksaan ");
                    ttljumlahp = ttljumlahp + jumlahp;

                    nilaitotall = Sequel.cariInteger(
                            "select SUM(CAST(detail_periksa_lab.nilai AS DECIMAL(10, 2))) AS nilai FROM template_laboratorium INNER JOIN detail_periksa_lab "
                                    + "ON template_laboratorium.id_template = detail_periksa_lab.id_template INNER JOIN reg_periksa ON detail_periksa_lab.no_rawat = reg_periksa.no_rawat INNER JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis "
                                    + "where detail_periksa_lab.tgl_periksa between '"
                                    + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' and '"
                                    + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' "
                    ...
                    jumlahpasienl = Sequel.cariInteger("SELECT count(periksa_lab.no_rawat) FROM periksa_lab INNER JOIN "
                            ...
                            + "where periksa_lab.tgl_periksa between '" + Valid.SetTgl(Tgl1.getSelectedItem() + "")
                            + "' and '" + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' "
                            + "and periksa_lab.kd_jenis_prw='" + rstindakan2.getString("kd_jenis_prw")
                            + "' and pasien.jk = 'L'");

                    nilaitotalp = Sequel.cariInteger(
                            "select SUM(CAST(detail_periksa_lab.nilai AS DECIMAL(10, 2))) AS nilai FROM template_laboratorium INNER JOIN detail_periksa_lab "
                                    + "ON template_laboratorium.id_template = detail_periksa_lab.id_template INNER JOIN reg_periksa ON detail_periksa_lab.no_rawat = reg_periksa.no_rawat INNER JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis "
                                    + "where detail_periksa_lab.tgl_periksa between '"
                                    + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' and '"
